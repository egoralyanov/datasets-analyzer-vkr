...
# Базовые нарративы по финальному типу задачи. Описывают, почему именно этот
# тип задачи подходит данным и какие классы алгоритмов/метрик используются.
# Текст намеренно не повторяет описания правил из applied_rules — на странице
# результата applied_rules идут отдельным блоком как «техническая» сводка
# сработавших ветвей дерева решений.
_TASK_TYPE_NARRATIVES = {
    "BINARY_CLASSIFICATION": (
        "Целевая переменная принимает два значения. Это типовая задача "
        "бинарной классификации: подойдут логистическая регрессия, "
        "случайный лес и градиентный бустинг; качество оценивается "
        "accuracy, F1, precision/recall и ROC-AUC."
    ),
    "MULTICLASS_CLASSIFICATION": (
        "Целевая переменная содержит несколько дискретных классов. "
        "Подойдут многоклассовые алгоритмы (логистическая регрессия "
        "one-vs-rest, случайный лес, градиентный бустинг); для оценки "
        "качества рекомендуется f1_macro и стратифицированное "
        "разбиение на кросс-валидационные фолды."
    ),
    "REGRESSION": (
        "Целевая переменная — непрерывная числовая величина. Подойдут "
        "регрессионные модели (линейная и Ridge-регрессия, случайный "
        "лес, градиентный бустинг); качество оценивается метриками "
        "MAE, RMSE и коэффициентом детерминации R²."
    ),
    "CLUSTERING": (
        "Целевая переменная не указана — задача относится к обучению "
        "без учителя. Подойдут алгоритмы кластеризации (KMeans, DBSCAN, "
        "иерархическая кластеризация); качество оценивается силуэтом "
        "и индексом Дэвиса–Болдина."
    ),
    "NOT_READY": (
        "Данные в текущем виде непригодны для обучения модели — "
        "требуется доработка целевого столбца или предобработка "
        "признаков."
    ),
}


# Уточнения, добавляемые к базовому нарративу при срабатывании конкретных
# правил. Привязка идёт по коду правила, а не по его описанию — это делает
# explanation смысловым «слоем» поверх applied_rules, а не их пересказом.
_RULE_NARRATIVE_MODIFIERS: dict[str, str] = {
    "BINARY_IMBALANCED": (
        " Классы в выборке заметно несбалансированы — при обучении "
        "следует опираться на F1-меру или ROC-AUC вместо accuracy и "
        "применять балансировку (oversampling, class_weight='balanced')."
    ),
    "BINARY_BALANCED": (
        " Классы распределены примерно поровну, поэтому дополнительная "
        "балансировка не требуется."
    ),
    "NO_TARGET_HIGH_DIM": (
        " Признаков относительно много — перед запуском алгоритмов "
        "кластеризации полезно понизить размерность методом главных "
        "компонент (PCA) или нелинейным embedding'ом (UMAP, t-SNE)."
    ),
    "ONLY_ONE_CLASS": (
        " В целевом столбце встречается только один класс — "
        "обучить классификатор невозможно. Проверьте, правильно ли "
        "указан target и есть ли в данных вариативность."
    ),
    "TARGET_LOOKS_LIKE_ID": (
        " Целевая переменная похожа на идентификатор: уникальных "
        "значений почти столько же, сколько строк. В таком виде target "
        "не описывает класс — выберите другой столбец или сформулируйте "
        "задачу иначе."
    ),
}
...
def _build_explanation(
    ...
    *,
    ...
    """
    Собирает русский нарратив «Почему такая рекомендация» для UI.

    В отличие от applied_rules (технического списка сработавших правил,
    показываемого на странице отдельным блоком), explanation — это связное
    обоснование на естественном языке: какой тип задачи рекомендуется и
    почему он подходит данным. Тексты правил здесь намеренно не цитируются
    дословно, чтобы избежать дублирования: applied_rules и explanation
    идут на странице друг под другом.

    Структура:
    - Базовый нарратив, привязанный к финальному `final_task_type`
      (бинарная / многоклассовая / регрессия / кластеризация / not_ready).
    - Опциональные уточнения, привязанные к ключевым кодам правил
      (дисбаланс, ID-like target, размерность для кластеризации и т.д.).
    - Если применялся Слой 2 — мягкая формулировка про confidence ML.
    - Если модель недоступна — пометка про fallback на правила.
    """
    parts: list[str] = []

    base_narrative = _TASK_TYPE_NARRATIVES.get(final_task_type)
    if base_narrative is None:
        # Неизвестный final_task_type — деградируем к короткой сводке.
        ...
        applied_codes = {rule.code for rule in rules_result.applied_rules}
        modifiers = "".join(
            modifier
            for code, modifier in _RULE_NARRATIVE_MODIFIERS.items()
            if code in applied_codes
        )
        parts.append(base_narrative + modifiers)
    ...
    if source in {"ml", "hybrid"} and ml_class is not None:
        ...
        parts.append(
            f"Решение уточнено мета-классификатором Слоя 2: вероятность "
            f"типа «{ml_name_ru}» — {pct:.0f}%."
        )

    if model_unavailable:
        parts.append(
            "Сейчас используются только правила; ML-уточнение будет доступно "
            "после обучения модели (см. `make train-meta`)."
        )
