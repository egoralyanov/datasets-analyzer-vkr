...
----------------------------------------------------------------------------
Numeric-discrete bridge (Phase 7)
----------------------------------------------------------------------------
В Phase 7 в `apply_rules` добавлен мост, переинтерпретирующий numeric-
discrete target (метки `"0"/"1"/"2"`, малая кардинальность) из categorical
в regression. Это фикс симптома: `infer_target_kind` в профайлере с
порогом `max(20, 5%·n_rows)` для маленьких датасетов (Iris, n=150)
ошибочно метит target=0/1/2 как categorical, и AMBIGUOUS_NUMERIC_TARGET
→ Слой 2 ML становится недостижимым. Bridge восстанавливает этот путь.

Тесты, покрывающие bridge:
- `test_ambiguous_numeric_target_delegates_to_ml` — bridge срабатывает,
  Iris-подобная meta идёт в Слой 2 (source ∈ {hybrid, ml}).
- `test_numeric_label_bridge_does_not_affect_string_target` — на string-
  метках bridge НЕ срабатывает, Ветка 4 сохраняет поведение.
- `test_numeric_bridge_skips_too_many_unique` — при n_unique > 20 bridge
  НЕ срабатывает (high-cardinality numeric остаются в Ветке 4).
- `test_recommend_iris_returns_multiclass` — обновлён в Phase 7: теперь
...
def test_ambiguous_numeric_target_delegates_to_ml():
    """
    Phase 7 numeric-discrete bridge: target_kind="categorical" с числовыми
    метками `"0"/"1"/"2"` (Iris-подобная meta) → bridge переписывает
    target_kind на "regression" → Ветка 3 → AMBIGUOUS_NUMERIC_TARGET
    (n_unique=3 ≤ 10) с requires_ml=True. При наличии загруженной модели
    финальный source — "hybrid" или "ml".
    """
    meta = _make_meta(
        target_kind="categorical",
        target_n_unique=3,
        target_value_counts={"0": 50, "1": 50, "2": 50},
    )
    result = apply_rules(meta, target_column="target")
    assert result.requires_ml is True
    assert result.confidence == pytest.approx(0.5)
    codes = [r.code for r in result.applied_rules]
    assert "AMBIGUOUS_NUMERIC_TARGET" in codes

    # Полный путь recommend_task: проверяем, что Слой 2 действительно
    # активируется (модель должна быть подгружена через автофикстуру
    # _ensure_meta_classifier_trained в conftest).
    rec = recommend_task(meta, target_column="target")
    assert rec.source in {"hybrid", "ml"}
    assert rec.ml_probabilities is not None


def test_numeric_label_bridge_does_not_affect_string_target():
    """
    Phase 7 numeric-discrete bridge не должен срабатывать на string-метках
    (например, Iris со species: setosa/versicolor/virginica). Ветка 4
    обрабатывает такой target как multiclass classification на 3 классах
    с min_class_size=50 → MULTICLASS confidence=0.95, requires_ml=False.
    """
    meta = _make_meta(
        target_kind="categorical",
        target_n_unique=3,
    ...
    result = apply_rules(meta, target_column="species")
    assert result.task_type_code == "MULTICLASS_CLASSIFICATION"
    assert result.confidence == pytest.approx(0.95)
    assert result.requires_ml is False
    assert "AMBIGUOUS_NUMERIC_TARGET" not in [r.code for r in result.applied_rules]


def test_numeric_bridge_skips_too_many_unique():
    """
    Phase 7 numeric-discrete bridge ограничен диапазоном [3, 20] по
    n_unique. При n_unique=25 (high-cardinality numeric, который
    осмысленно остаётся в Ветке 4) bridge НЕ срабатывает —
    HIGH_CARDINALITY_MULTICLASS отрабатывает как и раньше.
    """
    n_unique = 25
    meta = _make_meta(
        n_rows=200,
        ...
        target_value_counts={str(i): 8 for i in range(n_unique)},
    )
    result = apply_rules(meta, target_column="cls_id")
    codes = [r.code for r in result.applied_rules]
    # Bridge не сработал → не идём в numeric-ветку.
    assert "AMBIGUOUS_NUMERIC_TARGET" not in codes
    # Остаёмся в Ветке 4: cardinality_ratio=25/200=0.125 ≤ 0.5 →
    # HIGH_CARDINALITY_MULTICLASS (а не TARGET_LOOKS_LIKE_ID).
    assert "HIGH_CARDINALITY_MULTICLASS" in codes
    assert result.task_type_code == "MULTICLASS_CLASSIFICATION"
...
def test_recommend_iris_returns_multiclass():
    """
    E2E smoke: meta из реального Iris (3 класса) → MULTICLASS через
    numeric-discrete bridge (Phase 7).

    После Phase 7 Iris с числовым target=0/1/2 проходит numeric-discrete
    bridge и попадает в Ветку 3 (AMBIGUOUS_NUMERIC_TARGET, requires_ml=True),
    активируя Слой 2. Финальный source — "hybrid" (правила выдали
    ...
    assert rec.source in {"hybrid", "ml"}
    assert rec.ml_probabilities is not None
    # ML-вероятность для MULTICLASS должна быть наибольшей.
    probs = rec.ml_probabilities
    assert probs["MULTICLASS_CLASSIFICATION"] == max(probs.values())
